...
import { TopNavElement, TopNavWrapper } from "component/common/Navbar/TopNav/index";
...
const SearchBarWrapper = styled(TopNavWrapper)`
  top: ${({ $isSearchOpened }) => $isSearchOpened ? 0 : "-" + TOP_NAVBAR_HEIGHT};
...
export default function SearchBar({ handleClose, $isSearchOpened }) {
  ...
  const [_isSearching, searchResult, _searchError, search] = useAPI(req.search.user);
  ...
  useEffect(() => {
    ...
    inputRefSnapshot.addEventListener("focusout", hideResults);

    return () => {
      inputRefSnapshot.removeEventListener("focusin", showResults);
      inputRefSnapshot.removeEventListener("focusout", hideResults);
    }
  ...
  return (
    <>
      <SearchBarWrapper as="div" $isSearchOpened={$isSearchOpened}>
        <TopNavElement.GoBackButton onClick={handleClose} title="계정 검색바 닫기" />
        <label htmlFor="serch-input" className="sr-only">계정 검색바</label>
        <SearchInput
          id="serch-input"
          ref={inpSearchRef}
          type="text"
      ...
      {isSearchResultExists && <ResultListWrapper visible={shouldShowResults && isInputExists}>
